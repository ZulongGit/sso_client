...
<script type="text/javascript">
	...
	function beforeEditName(treeId, treeNode) {
		$.cuslayer({
			...
			width:"${width}",
			url:"${ctxPath!}/"+"${editUrl}",
			data:{"id":treeNode.id,"parentId":treeNode.getParentNode().${idKey!'id'}}
		});
		return false;
	}
	...
	function addHoverDom(treeId, treeNode) {
		...
		if (btn) btn.bind("click", function(){
			$.cuslayer({
				...
				width:"${width}",
				url:"${ctxPath!}/"+'${addUrl}',
				data:{"parentId":treeNode.${idKey!'id'}}
			});
			return false;
		});
	};
	...
	function onClickNode(event, treeId, treeNode) {
		...
		$("#${form}").find("input[name=code]").val(treeNode.code);
		query(); //刷新表单
